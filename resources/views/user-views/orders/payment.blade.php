            ...
            <div class="bg-neutral-950 text-white min-h-screen p-6 sm:p-10">
                ...
                <div class="mt-5 px-4 lg:px-24">
                    <div class="flex flex-col md:flex-row gap-6">
                        <!-- Left Section -->
                        <div
                            class="w-full md:w-1/2 bg-neutral-800 bg-opacity-90 p-4 sm:p-6 rounded-lg shadow-lg">
                            <div class="flex items-center gap-4 mb-6">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor" class="w-6 h-6 sm:w-8 sm:h-8 text-amber-300">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                                </svg>
                                <h2 class="text-xl sm:text-2xl md:text-3xl font-semibold text-white">Order #{{ $orders->id }}</h2>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 border-b pb-4">
                                <div class="flex justify-between">
                                    ...
                                    <span class="text-sm sm:text-base text-white font-semibold">{{ $orders->user->name }}</span>
                                </div>
                                <div class="flex justify-between">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <span class="text-sm sm:text-base text-white font-semibold">{{ $orders->table_number ?? '-' }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-sm sm:text-base text-gray-300">Voucher Name:</span>
                                    <span class="text-sm sm:text-base text-white font-semibold">{{ $orders->voucher->name ?? '-' }}</span>
                                </div>
                            </div>
                            ...
                            <div class="mt-4 space-y-4">
                                @foreach ($orders->menus as $order)
                                    ...
                                    @endphp
                                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-neutral-700 p-4 rounded-lg">
                                        <div class="flex items-center gap-4">
                                            <img src="images/tuna.jpg" alt="Item Image"
                                                class="h-16 w-16 object-cover rounded-lg">
                                            <div>
                                                <h3 class="text-base sm:text-lg font-medium text-white">{{ $order->name }}</h3>
                            ...
                            <!-- Checkout Button -->
                            <a href="{{ route('user.orders.pay', $orders->id) }}">
                        ...
                        <div id="snap-container"
                            class="w-full md:w-1/2 p-4 sm:p-6 shadow-lg rounded-lg mt-6 md:mt-0">
                        </div>
                    </div>
                </div>
            </div>
